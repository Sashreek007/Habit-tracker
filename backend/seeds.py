...
with app.app_context():
    ...
    # Add a habit for user1 if it doesn't exist
    ...
    if not habit1:
        habit1 = Habit(name="Workout", user_id=user1.id)
        db.session.add(habit1)
        db.session.commit()
    if not habit2:
        habit1 = Habit(name="Dance", user_id=user1.id)
        db.session.add(habit1)
        db.session.commit()

    # Add a log for today if not already added
